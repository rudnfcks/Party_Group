    ...
    return (
        <HomeConteiner>
            ...
            <section id="top">
                ...
                <div id="count">
                    <p>이번 달의 파티 <span><b>{0}</b> 회</span></p>
                    <p>이번 주의 파티 <span><b>{0}</b> 회</span></p>
                </div>
            </section>
            ...
            <section id="content">
                {
                    // 기간이 남은 리스트
                    store.partys !== null ? (
                        store.partys.filter((item) => (new Date(item.dateTime) >= date)).map((item) => (
                            <List key={item.id} listId={item.id} info={item} name={getCookie("name")} code={getCookie("code")} />
                        ))
                    ) : (
