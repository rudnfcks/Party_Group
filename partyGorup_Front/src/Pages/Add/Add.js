...
function Add({setPage}) {
    ...
    const submitParty = () => {
        ...
        if (place === "" || dateTime === undefined || count === undefined) {
            alert("모두 입력해주세요!")
            return
        }
    ...
    return (
        <BackgroundGray>
            <InputContainer>
                <div id="header">
                    <Link to="/page/home" replace onClick={() => {setPage("home")}}>
                ...
                <div id="section">
                    <Input label="장소" placeholder="장소 입력" type="text" value={inputInfo.place} onChange={(value) => {inputChange("place", value)}} />
                    <DateInput value={inputInfo.dateTime} onChange={(value) => {inputChange("dateTime", value)}} />
                    <Input label="인원" placeholder="인원 입력" type="number" value={inputInfo.count} onChange={(value) => {inputChange("count", value)}} />
                </div>
            </InputContainer>
        </BackgroundGray>
    )
